...
| 2     | Узгодження захищеного каналу (TLS/SSL)       | * TLSv1.3 (OUT), TLS handshake...      |  Рядки демонструють етап перевірки сертифікатів і встановлення шифрування перед тим, як дані почнуть передаватися.       |
| 3   |  Обмін прикладними даними (HTTP)    | > GET / HTTP/2,< HTTP/2 200, > Host: sqlite.org    |  Ці рядки відповідають за безпосередній запит контенту вебсторінки та отримання відповідей у зрозумілому для клієнта текстовому форматі.     |
| 4     | Узгодження захищеного каналу (TLS/SSL)       | * TLSv1.3 (OUT), TLS handshake...      |  Рядки демонструють етап перевірки сертифікатів і встановлення шифрування перед тим, як дані почнуть передаватися.       |
